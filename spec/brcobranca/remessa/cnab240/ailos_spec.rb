...
RSpec.describe Brcobranca::Remessa::Cnab240::Ailos do
  ...
  let(:ailos) { subject.class.new(params) }
  ...
  context 'validacoes' do
    context '@digito_agencia' do
      ...
      it 'deve ser invalido se o digito da agencia nao tiver 1 digito' do
        ailos.digito_agencia = '12'
        expect(ailos.invalid?).to be true
        expect(ailos.errors.full_messages).to include('Digito agencia deve ter 1 dígito.')
      end
    end
    ...
    context '@convenio' do
      ...
      it 'deve ser invalido se o convenio tiver mais de 6 digitos' do
        ailos.convenio = '1234567'
        expect(ailos.invalid?).to be true
        expect(ailos.errors.full_messages).to include('Convenio deve ter 6 dígitos.')
      end
    end
    ...
    context '@conta_corrente' do
      ...
      it 'deve ser invalido se o conta corrente tiver mais de 7 digitos' do
        ailos.conta_corrente = ''.rjust(8, '0')
        expect(ailos.invalid?).to be true
        expect(ailos.errors.full_messages).to include('Conta corrente deve ter 7 dígitos.')
      end
    end
  end
  ...
  context 'formatacao' do
    it 'codigo do banco deve retornar 085' do
      expect(ailos.cod_banco).to eq '085'
    end
    ...
    it 'versao do layout do arquivo deve retornar 087' do
      expect(ailos.versao_layout_arquivo).to eq '087'
    end

    it 'versao do layout do lote deve ser 045' do
      expect(ailos.versao_layout_lote).to eq '045'
    end

    it 'codigo do convenio deve ser 20 caracteres' do
      expect(ailos.codigo_convenio).to eq '123456'.ljust(20, ' ')
    end

    it 'convenio lote deve retornar as informacoes nas posicoes corretas' do
      conv_lote = ailos.convenio_lote
    ...
    it 'complemento header deve retornar as informacoes nas posicoes corretas' do
      comp_header = ailos.complemento_header
    ...
    it 'complemento P deve retornar as informacoes nas posicoes corretas' do
      comp_p = ailos.complemento_p pagamento
    ...
    it 'tipo do documento deve ser 1 - Tradicional' do
      expect(ailos.tipo_documento).to eq '1'
    end
